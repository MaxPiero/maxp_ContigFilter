...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'maxp_ContigFilter'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from maxp_ContigFilter.maxp_ContigFilterImpl import maxp_ContigFilter
impl_maxp_ContigFilter = maxp_ContigFilter(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['maxp_ContigFilter.filter_contigs_async'] = ['maxp_ContigFilter', 'filter_contigs']
async_check_methods['maxp_ContigFilter.filter_contigs_check'] = ['maxp_ContigFilter', 'filter_contigs']
sync_methods['maxp_ContigFilter.filter_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'maxp_ContigFilter'
        ...
        self.rpc_service.add(impl_maxp_ContigFilter.filter_contigs,
                             name='maxp_ContigFilter.filter_contigs',
                             types=[dict])
        self.method_authentication['maxp_ContigFilter.filter_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "maxp_ContigFilter but no authentication header was passed"
                            raise err
